...
static void *tcp_thread_send(void *fd);
...
SERVER_RETURN:
	...
	while(1) {
		...
		if(FD_ISSET(sock_listen, &rfds)) {
			...
			pthread_create(&thread_id_recv, &attr_recv, tcp_thread_recv, (void *)&sock_num);
...
static void *tcp_thread_recv(int *fd)
{
	...
	unsigned char out[RECV_BUF_SIZE] = {0};
	int inlen, outlen;
	int nLen;
	int num = 0;
...
void log_send_to_client(char *buf, int buf_len)
{
	int i ;
	int length = 0;
...
void server_send_audio_test(char *buf, int buf_len)
{
	int i ;
	int length = 0;
	char temp[RECV_BUF_SIZE] = {0};
	...
	for(i = 0 ; i < SOCK_NUM ; i++) {
		if(g_client_param.clidata[i].login == 1 && 	g_client_param.clidata[i].socket != 0) {
			WriteData(g_client_param.clidata[i].socket, temp, length);
			printf("i will send the audio buf len==%u= %d= %d\n", (mid_clock() - g_start_time), g_audio_num++, buf_len);
...
int log_porting_init(void *func)
{
	g_func = func;
	return 0;
}
