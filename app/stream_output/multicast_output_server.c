...
static int mult_set_mtu(int type, int mtu);
static int mult_set_active(int type, int status);
...
static int multicast_node_config(stream_output_server_config *config, MULTICAST_Output_Server_NODE *node)
{
	...
	node->client = client;
	stream_client_open(&video_addr, &audio_addr, config->type, client);
...
#if 0
static int multicast_client_create(struct sockaddr_in *video_addr,	struct sockaddr_in *audio_addr, int type, Stream_Output_Client *client)
{
...
static int multicast_client_config(Stream_Output_Client *client, stream_output_server_config *config)
{
...
static int multicast_client_delete(Stream_Output_Client *client)
{
	if(client == NULL) {
		ERR_PRN("\n");
		return -1;
...
#if 0
	ret = app_ini_read_int(config_file, table, "used", &value);
...
static void app_check_list_mult_status()
{
	...
	int direct_ts_status = 0;
	int direct_rtp_status = 0;
	int rtp_ts_status = 0;

	int direct_ts_mtu = 1450;
	int direct_rtp_mtu = 1450;
	int rtp_ts_mtu = 1450;
	...
	while(node != NULL) {
		if((node->s_used == ISUSED) && (node->config.type == TYPE_TS)
		   && (node->config.status == S_ACTIVE)) {
			...
			if(direct_ts_mtu > node->config.mtu) {
				direct_ts_mtu = node->config.mtu;
			}
		}
		...
		if((node->s_used == ISUSED) && (node->config.type == TYPE_RTP)
		   && (node->config.status == S_ACTIVE)) {
			...
			if(direct_rtp_mtu > node->config.mtu) {
				direct_rtp_mtu = node->config.mtu;
			}
		}
		...
		if((node->s_used == ISUSED) && (node->config.type == TYPE_TS_OVER_RTP)
		   && (node->config.status == S_ACTIVE)) {
			...
			if(rtp_ts_mtu > node->config.mtu) {
				rtp_ts_mtu = node->config.mtu;
			}
		}
	...
	mult_set_active(TYPE_TS, direct_ts_status);
	mult_set_active(TYPE_RTP, direct_rtp_status);
	mult_set_active(TYPE_TS_OVER_RTP, rtp_ts_status);

	mult_set_mtu(TYPE_TS , direct_ts_mtu);
	mult_set_mtu(TYPE_RTP , direct_rtp_mtu);
	mult_set_mtu(TYPE_TS_OVER_RTP , rtp_ts_mtu);
...
#ifdef TEST
void test_set_old_mult(char *ip, int port, int type)
{
	MultAddr test_addr ;
	memset(&test_addr, 0, sizeof(MultAddr));
	strcpy(test_addr.chIP, ip);
	test_addr.nPort = port;
	test_addr.nStatus = 1;
	PRINTF("type=%d\n", type);

	if(type != 2) {
		SetEStoTSMultPort(&test_addr, type);
	}

	protocal_test(type, &test_addr);
	return ;
}
#endif
...
static int g_direct_rtp_active = 0;
static int g_direct_ts_active = 0;
static int g_rtp_over_ts_active = 0;
int mult_rtp_get_active()
{
	if(g_direct_rtp_active > 0) {
		return 1;
	} else {
		return 0;
	}
...
int mult_ts_get_active()
{
	if(g_direct_ts_active > 0) {
		return 1;
	} else {
		return 0;
	}
...
int mult_rtpts_get_active()
{
	if(g_rtp_over_ts_active > 0) {
		return 1;
	} else {
		return 0;
	}
...
{
	if(type == TYPE_TS) {
		g_direct_ts_active = status;
	} else if(type == TYPE_RTP) {
		g_direct_rtp_active = status;
	} else if(type == TYPE_TS_OVER_RTP) {
		g_rtp_over_ts_active = status;
	}

	return 0;
}

static int g_ts_mtu = 1450;
static int g_rtp_mtu = 1450;
static int g_rtpts_mtu = 1450;
int mult_get_direct_rtp_mtu()
{
	return g_rtp_mtu;
}
int mult_get_direct_ts_mtu()
{
	return g_ts_mtu;
}
int mult_get_rtp_ts_mtu()
{
	return g_rtpts_mtu;
}
