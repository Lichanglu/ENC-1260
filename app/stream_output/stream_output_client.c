...
int stream_client_open(struct sockaddr_in *video_addr, struct sockaddr_in *audio_addr, int type, Stream_Output_Client *client)
{
	if(client == NULL) {
		PRINTF("Error\n");
		return -1;
	...
	PRINTF("client create is ok,the client is %d.\n", client->num);
	return 0;
}
