...
int app_add_header_info(char *buf)
{
	...
	if(!strcmp(g_board_type, "CL4000_DVI")) {
		...
		PRINTF("please app_add_header_info!!!\n");
	}
...
int app_add_extern_buff(char *name)
{
	...
	char enc1100_first_head[8] = {0x7E, 0x7E, 0x7E, 0x7E, 0x31, 0x31, 0x30, 0x30};
	char enc1200_first_head[8] = {0x7E, 0x7E, 0x7E, 0x7E, 0x48, 0x45, 0x4E, 0x43};

	if(!strcmp(g_board_type, "ENC1100")) {
		first_head = enc1100_first_head;
	} else {
		first_head = enc1200_first_head;
	}
	...
	PRINTF("22the old file  len is %ld.\n", fp_old_len);
