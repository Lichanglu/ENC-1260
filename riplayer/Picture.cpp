...
BOOL CPicture::Load(LPBYTE pData,int nLen)
{
	CMemFile file(pData, nLen);
	BOOL bRet = Load(file);
...
//
BOOL CPicture::Render(CDC* pDC, CRect rc, LPCRECT prcMFBounds,BOOL bOver) const
{
	ASSERT(pDC);
	...
	m_spIPicture->Render(*pDC, rc.left, rc.top, rc.Width(), rc.Height(),
		0, bOver ? hmHeight : 0, hmWidth, bOver ? -hmHeight : hmHeight, prcMFBounds);
